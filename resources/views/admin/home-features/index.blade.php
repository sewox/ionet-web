...
<div class="p-8">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Ana Sayfa Özellikleri</h1>
            <p class="text-gray-500 mt-1">Ana sayfada gösterilen özellik kartlarını yönetin</p>
        </div>
    ...
    <div class="mb-6 bg-white rounded-lg shadow p-4">
        <form method="GET" action="{{ route('admin.home-features.index') }}" class="flex gap-4">
            <div class="flex-1">
                <input 
                    type="text" 
                    name="search" 
            ...
            @if(request('search'))
                <a href="{{ route('admin.home-features.index') }}" class="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors">
                    Sıfırla
                </a>
            @endif
        </form>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($features as $feature)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-700 font-semibold rounded-full">
                                {{ $feature->order_index }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            @if($feature->icon)
                                <code class="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-mono rounded">
                                    {{ Str::limit($feature->icon, 30) }}
                                </code>
                            @else
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" 
                                      action="{{ route('admin.home-features.destroy', $feature->id) }}" 
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                <span class="material-symbols-outlined text-gray-300 text-[48px]">featured_play_list</span>
                                <p class="text-gray-500">Henüz özellik bulunmuyor</p>
                                <a href="{{ route('admin.home-features.create') }}" 
